...
| **Address resolution** | SOCKS proxy address resolved reactively over the LXC bridge via `sdk.host.getBridgeAddress` (`sdk.host.get` on Tor's `socks` host, `.const()`); install/run state tracked via `sdk.getStatus`                                     |
